...
typedef struct bb_chunkdesc_struct {
    uint32_t type;
    uint32_t len;
    uint32_t startpos; /* start of chunk header */
    uint32_t datpos; /* start of data (either startpos or startpos+8) */
...
struct bb_map_struct {
    uint32_t inited; /* holds bb_Inited_Magic if the map structure is valid */
    FILE *file;
...
extern char *bb_id_to_string(uint32_t id);
