...
static void dumb_getline(char *s)
{
    ...
    while (p < s + INPUT_BUFFER_SIZE - 1) {
      *p = xgetchar();
      if (*p == '\r') { // we are usually in raw mode on rosco
        putchar('\r');
        putchar('\n');
        p++;
        *p = '\0';
      ...
      if (*p == '\b') { // crudely handle backspace
        if (p != s) {
          putchar('\b');
          putchar(' ');
          putchar('\b');
...
static void translate_special_chars(char *s)
{
  ...
  while (*src)
    ...
    case '\\':
      switch (*src++) {
      case '\n': *dest++ = (char)ZC_RETURN; break;
      case '\\': *dest++ = '\\'; break;
      case '?': *dest++ = (char)ZC_BACKSPACE; break;
      case '[': *dest++ = (char)ZC_ESCAPE; break;
      case '_': *dest++ = (char)ZC_RETURN; break;
      case '^': *dest++ = (char)ZC_ARROW_UP; break;
      case '.': *dest++ = (char)ZC_ARROW_DOWN; break;
      case '<': *dest++ = (char)ZC_ARROW_LEFT; break;
      case '>': *dest++ = (char)ZC_ARROW_RIGHT; break;
      case 'R': *dest++ = (char)ZC_HKEY_RECORD; break;
      case 'P': *dest++ = (char)ZC_HKEY_PLAYBACK; break;
      case 'S': *dest++ = (char)ZC_HKEY_SEED; break;
      case 'U': *dest++ = (char)ZC_HKEY_UNDO; break;
      case 'N': *dest++ = (char)ZC_HKEY_RESTART; break;
      case 'X': *dest++ = (char)ZC_HKEY_QUIT; break;
      case 'D': *dest++ = (char)ZC_HKEY_DEBUG; break;
      case 'H': *dest++ = (char)ZC_HKEY_HELP; break;
...
bool dumb_handle_setting(const char *setting, bool show_cursor, bool startup)
{
    if (!strncmp(setting, "sf", 2)) {
      speed = 1.0;
      printf("Speed not handled correctly for rosco and set to 1.0\n");
...
{
  // HACK - remove time functions for now 
  // time_t start_time;
  ...
  // printf("In dumb_read_line()\n");
  for (;;) {
  // printf("In dumb_read_line() loop\n");

    char *command;
    if (prompt) {
    // printf("In dumb_read_line() just before fputs()\n");
      printf("%s", prompt);
      //fputs(prompt, stdout);
    }
    else {
    // printf("In dumb_read_line() before dumb_show_prompt()\n");
      dumb_show_prompt(show_cursor, (timeout ? "tTD" : ")>}")[type]);
    }
    /* Prompt only shows up after user input if we don't flush stdout */
    // printf("In dumb_read_line() about to call fflush()\n");
  
    fflush(stdout);
    // printf("In dumb_read_line() about to call dumb_getline()\n");
    dumb_getline(s);
    // printf("In dumb_read_line() called dumb_getline()\n");
...
zchar os_read_key (int timeout, bool show_cursor)
{
  ...
  if (read_key_buffer[0] == '\0') {
    // HACK - remove time functions for now 
    // timed_out = 
    (void)dumb_read_line(read_key_buffer, NULL, show_cursor, timeout,
			       INPUT_CHAR, NULL);
...
zchar os_read_line (int UNUSED (max), zchar *buf, int timeout, int UNUSED(width), int continued)
{
  ...
  if (read_line_buffer[0] == '\0')
    // HACK - remove time functions for now 
    // timed_out = 
    (void)dumb_read_line(read_line_buffer, NULL, TRUE, timeout,
