...
typedef struct bb_result_struct {
    ...
    union {
        ...
        uint32_t startpos; /* The position in the file (if you used bb_method_FilePos) */
    } data;
...
typedef struct bb_aux_pict_struct {
    uint32_t ratnum, ratden;
    uint32_t minnum, minden;
    uint32_t maxnum, maxden;
} bb_aux_pict_t;
...
typedef struct bb_zheader_struct {
    uint16_t releasenum; /* Bytes $2-3 of header. */
    char serialnum[6]; /* Bytes $12-17 of header. */
    uint16_t checksum; /* Bytes $1C-1D of header. */
...
extern bb_err_t bb_load_chunk_by_type(bb_map_t *map, int method,
    bb_result_t *res, uint32_t chunktype, int count);
...
extern bb_err_t bb_load_resource(bb_map_t *map, int method,
    bb_result_t *res, uint32_t usage, int resnum);
extern bb_err_t bb_count_resources(bb_map_t *map, uint32_t usage,
...
extern uint16_t bb_get_release_num(bb_map_t *map);
